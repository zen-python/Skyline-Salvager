...
var salvage_grid := SalvageGrid.new()
var salvage_beam := SalvageBeam.new()
var salvage_crates: Array[SalvageCrate] = []
...
func _spawn_demo_crates() -> void:
	salvage_crates = [
		SalvageCrate.new("crate_a", Vector2i(3, 0)),
		SalvageCrate.new("crate_b", Vector2i(2, 2)),
	]


func _current_crate() -> SalvageCrate:
...
func _pull_selected_crate() -> void:
	if salvage_beam.state != SalvageBeam.STATE_PULLING:
...
func _on_lock_acquired(crate: SalvageCrate) -> void:
...
func _on_pull_started(crate: SalvageCrate) -> void:
	_refresh_status("Pull started for %s." % crate.crate_id)


func _on_pull_step_completed(crate: SalvageCrate, from_tile: Vector2i, to_tile: Vector2i) -> void:
