#!/bin/bash
# Codex session helper: load project context at session start.
# Run manually from the Codex scaffold root.

echo "=== OpenAI Codex Game Studios — Session Context ==="
...
if [ -d "production/qa/bugs" ]; then
    BUG_COUNT=$(find "production/qa/bugs" -name "BUG-*.md" 2>/dev/null | wc -l)
fi
