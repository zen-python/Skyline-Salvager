...
with a Linux `arm64` downloadable build artifact.
...
- **Planned Release at Kickoff**: `v0.2.0`
...
| S02-005 | Publish production GitHub release | producer | 0.5 | S02-004 | `Publish Release` is dispatched with `release_type=minor` and `prerelease=false`, producing a public GitHub Release with Linux `arm64` assets attached | Complete |
...
- The first planned target was `v0.2.0`, but the actual published release on
  2026-04-13 became `v0.3.0` because `v0.2.0` already existed on the remote
  release history when the publish workflow recomputed the next minor version.
